...
## Interstitial
* <span style='color: #fff5b1'><주의> 먼저 Initialize를 진행을 완료해야 합니다.</span>
* <span style='color: #fff5b1'><주의> Adiscope.framework외에 사용할 Network를 연동해야 합니다. [(연동하기)](https://github.com/adiscope/Adiscope-iOS-Sample/blob/main/Installation_manual.md#installation)</span>

### 광고 요청 및 재생
* 다음의 코드로 광고를 요청 할 수 있습니다.
```swift
adiscopeSDK.loadInterstitial(interstitialUnitID)
```
...
* Interstitial에 해당하는 응답입니다. `setMainDelegate(_:)`를 진행했다면 다음과 같은 응답을 수신 받을 수 있습니다.
```swift
// Load Callbacks
...
### 광고 요청 및 재생
* 다음의 코드로 광고를 미리 요청 할 수 있습니다.
* 1회 호출 권장합니다.
```swift
adiscopeSDK.preLoadAllRewardedInterstitial()
```
```swift
adiscopeSDK.preLoadRewardedInterstitial([rewardedInterstitialUnitID1, rewardedInterstitialUnitID2, ...])
```

* Show요청으로 광고를 송출합니다.
```swift
adiscopeSDK.showRewardedInterstitial(rewardedInterstitialUnitID)
```

<br>

### RewardedInterstitial Callbacks
* Interstitial에 해당하는 응답입니다. `setMainDelegate(_:)`를 진행했다면 다음과 같은 응답을 수신 받을 수 있습니다.
```swift
// Show Callbacks
...
```
* <span style='color: #fff5b1'><주의> onRewardedInterstitialRewarded의 응답은 실제 지급에 대한 요청이 아닙니다. Adiscope는 Server-Side Callback을 지원하고 있습니다.</span>
  <br><br>
